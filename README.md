...
2. 配置**application-local.yml**文件:
   ...
   4. **MongoDB配置**: 修改application-local.yml文件中的mongodb配置,将mongodb的host,port,username,password,database修改为你自己的mongodb配置
...
- 数据库采用**事务操作** ,要么全部成功,要么全部失败,防止数据不一致的情况发生。
...
- 采取**MongoDB文档模型**是因为这个课程表**主要的业务是读取**
